...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate('/');
      window.location.reload(); // Force reload to refresh sidebar/state
    } catch (err) {
      ...
      >
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          <div style={{ width: 60, height: 60, borderRadius: '1rem', background: 'linear-gradient(135deg,#6366f1,#a855f7)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 1.5rem', boxShadow: '0 8px 16px rgba(99,102,241,0.3)' }}>
            <LogIn style={{ color: '#000', width: 30, height: 30 }} />
          </div>
          <h1 style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: '2rem', fontWeight: 700 }}>Connexion</h1>
          <p style={{ color: '#adaaaa', marginTop: '0.5rem' }}>Accédez à votre planificateur IA</p>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          ...
          >
            {loading ? <Loader2 style={{ animation: 'spin 1s linear infinite' }} /> : 'Se connecter'}
          </motion.button>
        </form>

        <p style={{ textAlign: 'center', marginTop: '1.5rem', color: '#adaaaa', fontSize: '0.875rem' }}>
          Vous n'avez pas de compte ? <Link to="/signup" style={{ color: '#6366f1', textDecoration: 'none', fontWeight: 600 }}>S'inscrire</Link>
        </p>
      </motion.div>
    </div>
  );
